module Api::V2::LookupKeysCommonController
  extend ActiveSupport::Concern
    ...
    def find_smart_variable
      ...
      @smart_variable   = LookupKey.authorized(:view_external_variables).smart_class.find_by_id(id.to_i) if id.to_i > 0
      @smart_variable ||= (puppet_cond = { :puppetclass_id => @puppetclass.id } if @puppetclass
                           LookupKey.authorized(:view_external_variables).smart_class.where(puppet_cond).find_by_key(id)
                          )
      @smart_variable
    end

    def find_smart_variables
      @smart_variables   = LookupKey.authorized(:view_external_variables).smart_class.search_for(*search_options).paginate(paginate_options) unless (@puppetclass || @host || @hostgroup)
      @smart_variables ||= if @puppetclass
                              LookupKey.authorized(:view_external_variables).global_parameters_for_class(@puppetclass.id).search_for(*search_options).paginate(paginate_options)
                           elsif @host || @hostgroup
                              puppetclass_ids  = (@host || @hostgroup).all_puppetclasses.map(&:id)
                              LookupKey.authorized(:view_external_variables).global_parameters_for_class(puppetclass_ids).search_for(*search_options).paginate(paginate_options)
                           end
      @smart_variables
    end
    ...
    def find_smart_class_parameters
      @smart_class_parameters   = LookupKey.authorized(:view_external_variables).smart_class_parameters.search_for(*search_options).paginate(paginate_options) unless (@puppetclass || @environment || @host || @hostgroup)

      @smart_class_parameters ||= if @puppetclass && @environment
                                    LookupKey.authorized(:view_external_variables).smart_class_parameters_for_class(@puppetclass.id, @environment.id)
                                  elsif @puppetclass && !@environment
                                    environment_ids = @puppetclass.environment_classes.pluck(:environment_id).uniq
                                    LookupKey.authorized(:view_external_variables).smart_class_parameters_for_class(@puppetclass.id, environment_ids).search_for(*search_options).paginate(paginate_options)
                                  elsif !@puppetclass && @environment
                                    puppetclass_ids = @environment.environment_classes.pluck(:puppetclass_id).uniq
                                    LookupKey.authorized(:view_external_variables).smart_class_parameters_for_class(puppetclass_ids, @environment.id).search_for(*search_options).paginate(paginate_options)
                                  elsif @host || @hostgroup
                                    puppetclass_ids = (@host || @hostgroup).all_puppetclasses.map(&:id)
                                    environment_id  = (@host || @hostgroup).environment_id
                                    LookupKey.authorized(:view_external_variables).parameters_for_class(puppetclass_ids, environment_id).search_for(*search_options).paginate(paginate_options)
                                  end
