module Hostext
  module Search
    ...
    included do
      ...
      scoped_search :in => :host_statuses, :on => :status, :offset => 0, :word_size => HostStatus::ConfigurationStatus::BIT_NUM*4, :rename => :'status.interesting', :complete_value => {:true => true, :false => false}
      scoped_search_status "applied",         :in => :host_statuses, :on => :status, :rename => :'status.applied'
      scoped_search_status "restarted",       :in => :host_statuses, :on => :status, :rename => :'status.restarted'
      scoped_search_status "failed",          :in => :host_statuses, :on => :status, :rename => :'status.failed'
      scoped_search_status "failed_restarts", :in => :host_statuses, :on => :status, :rename => :'status.failed_restarts'
      scoped_search_status "skipped",         :in => :host_statuses, :on => :status, :rename => :'status.skipped'
      scoped_search_status "pending",         :in => :host_statuses, :on => :status, :rename => :'status.pending'

      # TODO: add scoped search for the global status (ok/warn/error)
