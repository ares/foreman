module Orchestration::DNS
  ...
  def dns?
    ...
    host.managed? && managed? && hostname.present? && ip_available? && !domain.nil? && !domain.proxy.nil?
  end

  def reverse_dns?
    # host.managed? and managed? should always come first so that orchestration doesn't
    # even get tested for such objects
    host.managed? && managed? && hostname.present? && ip_available? && !subnet.nil? && subnet.dns?
  end
